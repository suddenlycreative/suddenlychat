                    ...
                    <x-admin.ui.table>
                        ...
                        <x-admin.ui.table.body>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div>
                                        @if ($coupon->type == Status::COUPON_TYPE_FIXED)
                                        <span>{{ showAmount(@$coupon->amount) }}</span>
                                        @else
                                        <span>{{ getAmount(@$coupon->amount) }}%</span>
                                        @endif
                                    </div>
                                </td>
                                <td>
                                    ...
                                    @else
                                    <x-admin.other.status_switch :status="$coupon->status"
                                        :action="route('admin.coupon.status', $coupon->id)" title="coupon" />
                                    @endif
                                </td>
                                <td>{{ strPlural(@$coupon->duration_days, 'day') }}</td>
                                <td>
                                    <a href="{{ route('admin.user.subscriptions') }}?coupon_id={{ $coupon->id}}">
                                        {{ $coupon->total_uses_count }} {{ str()->plural('Time',
                                        $coupon->total_uses_count) }}
                                    </a>
                                </td>
                                <td>
                                    <div data-coupon='@json($coupon)' data-duration="{{ @$coupon->duration }}">
        ...
        <form method="POST" class="row">
            ...
            <div class="form-group col-md-6">
                <label class="form-label">
                    ...
                    <span data-bs-toggle="tooltip"
                        title="@lang('The coupon code may only contain uppercase letters, numbers, underscores, and dashes (no spaces).')"><i
            ...
            <div class="form-group col-md-6">
                <label class="form-label">
                    ...
                    <span data-bs-toggle="tooltip" title="@lang('Enter value -1 if you don\'t for unlimited use.')"><i
                            class="las la-info-circle"></i></span>
                </label>
                <div class="input-group input--group">
                    <input class="form-control" type="number" name="use_limit" value="{{ old('use_limit') }}" required>
            ...
            <div class="form-group col-md-6">
                ...
                <div class="input-group input--group">
                    <input class="form-control" type="number" name="per_user_limit" value="{{ old('per_user_limit') }}"
                        required>
                    <span class="input-group-text">@lang('Times')</span>
                </div>
            </div>
            <div class="form-group col-md-6">
                ...
                <input name="date" type="search" class="date-picker form-control"
                    placeholder="@lang('Start Date - End Date')" autocomplete="off" value="" required>
            </div>
