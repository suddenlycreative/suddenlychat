...
trait InboxManager
{
    ...
    public function sendMessage(Request $request)
    {
        ...
        try {
            // ========= Prefixo do agente (somente envio manual no Inbox) =========
            try {
                $loggedUser = auth()->user();
                $agentName  = $loggedUser->fullname ?? $loggedUser->name ?? null;
                $current    = (string)($request->message ?? '');

                if (!empty($agentName) && $current !== '') {
                    // Formato: *_Nome:_*  (negrito + itálico) na primeira linha
                    $firstLine       = trim(strtok($current, "\n"));
                    $alreadyPrefixed = (bool)preg_match('/^\*_' . preg_quote($agentName, '/') . ':_\*/i', $firstLine);
                    if (!$alreadyPrefixed) {
                        $request->merge([
                            'message' => '*_' . $agentName . ':_*' . "\n" . $current,
                        ]);
                    }
                }
            } catch (\Throwable $e) {
                // não bloquear envio por erro de prefixo
            }
            // ====================================================================
